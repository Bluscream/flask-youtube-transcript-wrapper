...
def add_transcript(transcript_dict: dict[str, object], video_id: str, transcript: Transcript, formats: list[str]):
    transcript_dict.update({
        transcript.language: trans_dict(transcript, formats)
    }) # [transcript.language.title()] = trans_dict(transcript)

def add_video(ret_dict: dict, video_id: str, formats: list[str], translate = True):
    ...
    if translate and not "English" in transcript_dict.keys():
    ...
    ret_dict["results"][video_id] = transcript_dict
